    ...
    private function createSupportTiket($setup)
    {
        ...
        if (!$setup->tableExists($tableName)) {
            $table = $connection->newTable($setup->getTable($tableName))
                ...
                ->addColumn('ccrecipients', Table::TYPE_TEXT, 225, ['nullable' => true], 'CC Recipients')
                ->addColumn('productsku', Table::TYPE_TEXT, 225, ['nullable' => true], 'Product sku')
                ->addColumn('message', Table::TYPE_TEXT, 1000, ['nullable' => false], 'Message')
                ->addColumn('image', Table::TYPE_TEXT, null,[], 'Image')
